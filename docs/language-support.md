...
| `literalReflection` | The structurally decidable part of reflective dispatch becomes an edge. **Dispatch tables** (TS/JS only): `HANDLERS[k]()` / `HANDLERS["create"]()` over a module-private `const` object whose name is used nowhere in the file except its declaration, a type query, and as the receiver of an immediately invoked subscript; each entry binds by its same-file module-level function declaration, never by name (an imported entry is refused), all entries or none, and none over the synthesis fan-out cap. A Python module dict is never a table, because any importer can mutate it. **Self-typed receivers**: `this["m"]()`, `getattr(self, "m")()`, Ruby `send(:m)` — only in the lexical instance context of a class named once in its file (not a static or singleton method, a nested `function`, an object literal, a module, an `instance_eval` block, or a Python staticmethod/classmethod) — bind to the class's own or nearest ancestor's method plus every subclass override, when exactly one exists and no class involved has more than one parent. Edges carry `confidence: 'synthesized'` and `synthesizedBy: 'literal-reflective'`; every refusal stays a disclosed `dynamicBoundary` site with its reason; a subset rebuild binds nothing. Reflection by bare name on an untyped receiver and DI-container resolution are never resolved. Backed for TypeScript/JavaScript (both families) and Python/Ruby (self-typed receivers). | `supportsLiteralReflection()` over `DYNAMIC_BOUNDARY_LANG_SPECS` (`dynamic-boundary.ts`) |
...
`literalReflection` is cross-checked the same way in `literal-reflection.test.ts`: the claimed set must
equal the languages with a declared recovery rule, and each member must produce an edge on its fixture.
