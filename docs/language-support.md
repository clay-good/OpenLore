...
| `literalReflection` | The structurally decidable part of reflective dispatch becomes an edge: a module-level literal dispatch table (`HANDLERS[k]()`, `HANDLERS["create"]()`) declared once and never rebound or mutated in its file, and a literal member on a self-typed receiver (`this["m"]()`, `getattr(self, "m")()`, Ruby `send(:m)`) looked up in the enclosing class and its subclasses. A target binds only when exactly one internal candidate remains (no same-file preference); a table binds all of its entries or none, and one over the synthesis fan-out cap binds none. Edges carry `confidence: 'synthesized'` and `synthesizedBy: 'literal-reflective'`; every refusal stays a disclosed `dynamicBoundary` site with its reason. Reflection by bare name on an untyped receiver and DI-container resolution are never resolved. Backed for TypeScript/JavaScript/Python/Ruby. | `supportsLiteralReflection()` over `DYNAMIC_BOUNDARY_LANG_SPECS` (`dynamic-boundary.ts`) |
