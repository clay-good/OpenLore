...
Vue (`.vue`), Svelte (`.svelte`), and Astro (`.astro`) are recognized containers rather than
standalone language rows in the capability matrix. OpenLore extracts plain `<script>` blocks through
the JavaScript lane and `<script lang="ts">` blocks through the TypeScript lane. The surrounding file
is blanked without changing offsets or newlines, so functions, calls, signatures, CFG, and style facts
retain their true container-file positions. `get_language_support` accepts the format name or extension
(for example, `.svelte`) and returns the container extraction scope.
