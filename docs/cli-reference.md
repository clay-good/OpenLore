...
For an opt-in Claude Code Stop hook, run `openlore setup --agent-enforcement-hook claude`. The installer merges one idempotent entry into `.claude/settings.json`; `openlore setup --agent-enforcement-hook none` removes only that OpenLore entry. In agent-hook mode, remediation is printed before the finding conclusion when the code declares a static template. `frozen` stays advisory in the agent loop, and configuration or source failures emit caveats and exit 0.
