...
- **GIVEN** a cross-file call in an import-precise language versus a name-only-resolved language
- **WHEN** the conformance suite resolves each
- **THEN** the edge is found in every case
- **AND** the import-precise language's provenance is asserted as `import` while the remaining
  name-only languages' lower-confidence provenance is documented explicitly
...
> Decision recorded: 58cd7afe
> Date: 2026-08-30

#### Scenario: The decision requirement is enforced

- **GIVEN** approved decision `58cd7afe`
- **WHEN** the affected behavior is evaluated
- **THEN** The system SHALL atomically track spec changes and index build time, disclose bounded freshness metadata in spec search results, and clear the staleness receipt after a full analysis.
...
> Decision recorded: 9eb51001
> Date: 2026-08-30

#### Scenario: The decision requirement is enforced

- **GIVEN** approved decision `9eb51001`
- **WHEN** the affected behavior is evaluated
- **THEN** The system SHALL identify the scoring method used for every code and specification search result without removing or changing the existing score field.

### Requirement: SerializeSpecificationIndexAccessWithAnAdvisoryLock

The system SHALL acquire a shared bounded advisory lock for all specification-index reads and mutations, recover stale lock ownership, and report lock-acquisition failures clearly.

> Decision recorded: f61d6a80
> Date: 2026-08-30

#### Scenario: The decision requirement is enforced

- **GIVEN** approved decision `f61d6a80`
- **WHEN** the affected behavior is evaluated
- **THEN** The system SHALL acquire a shared bounded advisory lock for all specification-index reads and mutations, recover stale lock ownership, and report lock-acquisition failures clearly.

### Requirement: ApplyCodeSearchFiltersBeforeAnnCandidateSelection

The system SHALL apply supported language and minimum fan-in predicates before approximate-nearest-neighbor candidate limiting in vector code searches.

> Decision recorded: dc82420c
> Date: 2026-08-30

#### Scenario: The decision requirement is enforced

- **GIVEN** approved decision `dc82420c`
- **WHEN** the affected behavior is evaluated
...
> Decision recorded: fda1fc53
> Date: 2026-08-30

#### Scenario: The decision requirement is enforced

- **GIVEN** approved decision `fda1fc53`
- **WHEN** the affected behavior is evaluated
- **THEN** The system SHALL persist stale files and their structural composition atomically in the SQLite edge store for each graph generation.
...
SHALL stop and emit the partial lexicon marked `partial` with the omitted candidate-input count
...
This domain SHALL conform to the canonical statement of decision `fbf129f7`, which lives in the
`cli` domain — see [cli/spec.md](../cli/spec.md).

#### Scenario: The canonical statement governs

- **GIVEN** decision `fbf129f7` recorded in the `cli` domain
- **WHEN** this domain's behavior touches that decision's surface
- **THEN** it satisfies the canonical requirement as stated in [cli/spec.md](../cli/spec.md)

> Decision pointer: fbf129f7 — "Reuse watcher repository-delta semantics for ancestor bundle catch-up" is recorded in `openspec/specs/cli/spec.md`; it also affects this domain.
...
#### Scenario: Valid complex Java return type
- **GIVEN** a Java method whose return type contains deeply nested generics, type-use annotations, or arbitrarily spaced array dimensions
- **WHEN** the analyzer extracts public exports or resolves an HTTP route handler
- **THEN** the system SHALL report the method name without imposing an arbitrary token-count limit on the return type

#### Scenario: Malformed declaration input
- **GIVEN** an untrusted source file containing repeated unterminated Java generic prefixes, comment openers, or declaration-like tokens
- **WHEN** the analyzer scans the file for exports or HTTP route handlers
- **THEN** the system SHALL process the source with forward-only bounded work and SHALL NOT repeatedly rescan the same unterminated suffix

#### Scenario: Oversized dependency source
- **GIVEN** a dependency source file larger than the analyzer source-size limit
- **WHEN** dependency parsing is requested
- **THEN** the system SHALL skip import and export extraction and SHALL report that the file exceeded the limit
...
### Disclose spec index staleness through an atomic receipt
...
### Expose score semantics on search results
...
**Consequences:** Every search_code and search_specs result includes scoreKind, including literal-text fallbacks and hybrid retrieval, without changing the existing score field.

### Serialize specification index access with an advisory lock
...
### Apply code search filters before ANN candidate selection
...
### Persist stale-region composition in the edge store
...
**Consequences:** The stale receipt schema gains composition metadata, closure ordering remains a pure helper over resident node facts, and full rebuild or clear removes both stale files and composition together.
