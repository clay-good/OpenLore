...
> Source files: src/core/analyzer/java-method-scanner.ts, src/core/analyzer/comment-blanking.ts, src/core/services/secret-redaction.ts, src/core/services/llm-service.ts, src/core/services/config-schema.ts
...
- **GIVEN** approved decision `2d0457b5`
- **WHEN** the affected behavior is evaluated
- **THEN** The system SHALL redact repository secrets from all source-carrying tool outputs at shared dispatch by default while allowing trusted operators to opt out through configuration.
...
**Consequences:** The four source-carrying tools are redacted by default with typed, counted disclosure; trusted operators may opt out through configuration; existing error and telemetry redaction APIs remain compatible.
