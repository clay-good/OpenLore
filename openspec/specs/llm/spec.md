...
> Source files: src/core/services/llm-service.ts, docs/providers.md, src/types/index.ts, src/api/types.ts, src/utils/command-helpers.ts, openspec/specs/llm/spec.md, src/core/services/tls-scope.ts, src/core/services/repo-config-trust.ts, src/core/services/chat-agent.ts, src/core/analyzer/embedding-service.ts, src/core/analyzer/embedder.ts, src/cli/commands/doctor.ts, src/cli/commands/view.ts
...
> Decision recorded: b8520b68
> Date: 2026-08-28

#### Scenario: Relaxing one surface leaves the other verified

- **GIVEN** an operator opt-out set for the embedding surface only
- **WHEN** an LLM provider request is issued in the same process
- **THEN** that request still verifies the provider's certificate, and the reverse holds when
  only the LLM surface is opted out

#### Scenario: No single switch relaxes both surfaces

- **WHEN** the configuration surface is enumerated
- **THEN** it offers one opt-out per network surface and none that relaxes both at once
...
### Scope every TLS opt-out to one network surface; add no union switch
