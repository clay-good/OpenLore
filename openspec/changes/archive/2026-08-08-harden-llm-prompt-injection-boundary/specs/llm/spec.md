...
message rather than run under the user's tool allowlist. Restricted modes MAY retain
provider-supported read-only or search capabilities, which SHALL be disclosed.
...
- **THEN** the provider runs with tools disabled or its strongest documented restricted
  mode and cannot mutate the analyzed repository
