...
> `openlore enforce --agent-hook` mode shaped for Claude Code and Codex Stop-hook semantics,
...
- **The gate fires after the loop, not in it.** `openlore enforce --hook` is a *git pre-commit*
  ...
  option at `:324`). Claude Code and Codex Stop hooks have a different contract —
...
- **The installer plumbing already exists but is unused for this.** `openlore setup` writes
  agent hooks with idempotent merges and corrupt-settings refusal. The enforcement gate has no
  such wiring for Claude Code's `.claude/settings.json` or Codex's `.codex/hooks.json`.
...
1. **`openlore enforce --agent-hook`** (`src/cli/commands/enforce.ts` — a sibling mode to the
   ...
   per Claude Code and Codex Stop-hook semantics: exit 0 when nothing blocks (advisory findings summarized, one
...
2. **Installer wiring, on request.** `openlore setup` gains an opt-in flag to register the
   agent-hook as a Stop entry for Claude Code, Codex, or both, reusing corrupt-settings refusal
   and idempotent hook-entry merges — never clobbering user settings, never installed by default.
...
- Risk: exit-code contract drift if either host's Stop-hook semantics change (mitigated: the contract is
