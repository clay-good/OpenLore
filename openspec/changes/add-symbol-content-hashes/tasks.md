...
- [x] Normalized hash over each symbol's parse subtree on the extractor's existing parse
      ...
      excluded except Go directives, uncovered text hashed, residual hash + span order), opt-in via
...
- [x] Fallback disclosure: every file that stays file-granular is named with its reason
