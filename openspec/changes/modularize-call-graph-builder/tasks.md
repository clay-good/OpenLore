...
- [x] Re-export every moved symbol from `call-graph.ts` so no importer changes — slice 1 re-exports the public type/edge model (`RawEdge`/`CALL_DISTANCE_FALLBACK` kept internal); slices 3/3b moved file-internal helpers (no re-export needed); slice 3c re-exports the previously-public `computeCyclomaticComplexity`; repeated per later slice
- [x] Verify (slices 1, 3, 3b, 3c): zero edits to the 155 importers (tsc compiled them all clean); export surface byte-identical (multi-line-aware diff, no add/remove); analyzer + full suite green (279 files / 5534 tests); before/after snapshot byte-identical (slice 1 `131ba4c6…`, slice 3 `58107ac0…`, slice 3b `3a118017…`, slice 3c `7b765f31…`). `stable call-graph barrel` test locks the invariant for later slices.
