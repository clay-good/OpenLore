...
export const CAPABILITY_DESCRIPTIONS: Record<Capability, string> = {
  ...
  literalReflection: 'Literal reflective dispatch recovery under strict uniqueness: a stable module-private JS/TS `const` dispatch table (`HANDLERS[k]()`) whose entries bind by same-file declaration, and a literal member on a self-typed receiver in a class instance context (`this["m"]()`, `getattr(self, "m")()`, Ruby `send(:m)`), become `literal-reflective` synthesized edges; every refusal stays a disclosed dynamic-boundary site. Reflection by bare name on an untyped receiver is never resolved. A language WITHOUT this binds none of these shapes.',
};
