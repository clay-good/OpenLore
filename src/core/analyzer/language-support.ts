...
export const CAPABILITY_DESCRIPTIONS: Record<Capability, string> = {
  ...
  literalReflection: 'Literal reflective dispatch recovery: a stable module-private JS/TS `const` dispatch table (`HANDLERS[k]()`, `HANDLERS["create"]()`) whose entries bind by their same-file declarations becomes `literal-reflective` synthesized edges under strict uniqueness; every refusal stays a disclosed dynamic-boundary site. Self-typed receivers (`this["m"]()`, `getattr(self, "m")()`) and bare-name reflection are never resolved. A language WITHOUT this binds none of these shapes.',
};
