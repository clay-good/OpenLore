...
export const CAPABILITY_DESCRIPTIONS: Record<Capability, string> = {
  ...
  receiverResolution: 'Chained intra-object receiver resolution (`this.<field>.m()` / `self.<field>.m()`) through a per-file registry of declared field and return types. A language WITHOUT this leaves that call shape unresolved and disclosed as a boundary — never silently absent.',
