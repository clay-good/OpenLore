 ...
 *   attribute on a path changed by both sides, `merge.renormalize`, or a submodule conflict makes
 *   the pair `not-assessed`.
...
/** Paths changed by both sides that are checked for merge attributes. */
...
export function noLazyFetchEnv(): NodeJS.ProcessEnv {
  return { ...process.env, GIT_NO_LAZY_FETCH: '1' };
}
 ...
 * reason the merge cannot be simulated faithfully.
 */
async function repositoryMergeConfig(repoPath: string, deadline: number | undefined): Promise<{ args: string[] } | { detail: string }> {
  let listing: string;
  try {
    listing = await readGit(repoPath, ['config', '--get-regexp', '^(merge|diff)\\.(renames|renamelimit|directoryrenames|renormalize)$'], deadline);
  } catch (error) {
...
/**
 * A path changed by both sides whose `merge` attribute is not the default text merge, read from the
 * base, both tips, and the repository's own attributes files. Returns a detail, or undefined.
 */
async function mergeAttributeBlocker(repoPath: string, base: string, tipA: string, tipB: string, deadline: number | undefined): Promise<string | undefined> {
  ...
  const [a, b] = [await changed(tipA), await changed(tipB)];
  const shared = [...a].filter(path => b.has(path)).sort();
  if (shared.length === 0) return undefined;
  if (shared.length > SHARED_PATHS_CAP) return `${shared.length} paths changed on both sides exceed the ${SHARED_PATHS_CAP}-path merge-attribute check`;
...
async function simulate(repoPath: string, tipA: string, tipB: string, deadline: number | undefined, scratchParent: string): Promise<TextualMerge> {
  ...
  const [objectsDir, objectFormat] = (await readGit(repoPath, ['rev-parse', '--path-format=absolute', '--git-path', 'objects', '--show-object-format'], deadline))
    .split('\n').map(s => s.trim()).filter(Boolean);
  if (!objectsDir || !objectFormat) throw new Error('could not locate the object store');
