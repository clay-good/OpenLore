...
/**
 * Path-aware `readFile`: the dependency graph and the dynamic-boundary artifact are different
 * files, and this handler reads both.
 */
function mockArtifacts(sites: SiteSpec[], imports: Array<[string, string]> = []): void {
  ...
  vi.mocked(readFile).mockImplementation((async (p: unknown) => {
    const path = String(p);
    if (path.endsWith('dynamic-boundary.json')) return boundary;
    if (path.endsWith('dependency-graph.json')) return depGraph;
...
const run = () => handleFindDeadCode({ directory: '/p' }) as Promise<DeadResult>;
...
describe('find_dead_code — dynamic-boundary qualification', () => {
  ...
  it('names the specific site as the reason and caps the candidate at low', async () => {
    mockArtifacts([{ filePath: 'src/plugins.ts', language: 'TypeScript', kind: 'computed-member', line: 31 }]);
    const r = await run();
  ...
  it('does not lower a candidate a shipped cap already covers — and the reason gets specific', async () => {
    ...
    mockArtifacts([{ filePath: 'src/py/plugins.py', language: 'Python', kind: 'reflective-invoke', line: 7 }]);
  ...
  it('a site in another language never qualifies', async () => {
    mockArtifacts([{ filePath: 'src/plugins.ts', language: 'Python', kind: 'reflective-invoke', line: 3 }]);
