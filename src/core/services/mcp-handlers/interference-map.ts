...
export async function computeInterferenceMap(
  ...
  if (simulations > 0) {
    caveats.push('Textual merge verdicts come from a read-only `git merge-tree` simulation of the two tips with each other, not with the current base. It runs in a scratch repository, so no merge driver of this repository runs: a changed path with a non-default merge attribute, `merge.default`, `merge.renormalize`, branch merge options, replace refs or grafts, or a submodule conflict is not assessed, and rename settings are forwarded. Your global and system git config still apply.');
  }
